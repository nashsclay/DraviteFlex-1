...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop DraviteFlex server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "DraviteFlex server stopping";
}
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        {"draviteflex", "mnbudgetvoteraw", &mnbudgetvoteraw, true, true, false},
        {"draviteflex", "mnfinalbudget", &mnfinalbudget, true, true, false},
        {"draviteflex", "mnsync", &mnsync, true, true, false},
        {"draviteflex", "spork", &spork, true, true, false},
#ifdef ENABLE_WALLET
        {"draviteflex", "Darksend", &Darksend, false, false, true}, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
    ...
    if (((mapArgs["-rpcpassword"] == "") ||
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
                                             _("To use draviteflexd, or the -server option to draviteflex-qt, you must set an rpcpassword in the configuration file:\n"
                                               "%s\n"
                                               "It is recommended you use the following random password:\n"
                                               "rpcuser=draviteflexrpc\n"
                                               "rpcpassword=%s\n"
                                               "(you do not need to remember this password)\n"
                                               "The username and password MUST NOT be the same.\n"
                                               "If the file does not exist, create it with owner-readable-only file permissions.\n"
                                               "It is also recommended to set alertnotify so you are notified of problems;\n"
                                               "for example: alertnotify=echo %%s | mail -s \"DraviteFlex Alert\" admin@foo.com\n"),
...
{
    return "> draviteflex-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(string methodname, string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:22051/\n";
}
